...
function Awards(){ return <section id="awards" className="gray-section"><div className="section-heading"><p className="eyebrow">AWARDS & ACCREDITATIONS</p><h2>Recognition, compliance, and institutional credibility.</h2><p>Give For Society has received multiple recognitions for grassroots service and maintains key statutory registrations for transparent NGO operations.</p></div><div className="two-card-grid"><div className="card"><h3><ShieldCheck/> Awards & Recognition</h3>{awards.map(a=><div className="award" key={a.title}><strong>{a.title}</strong><span>{a.organization} • {a.year}</span></div>)}</div><div className="card"><h3><ClipboardCheck/> Accreditations & Compliance</h3><ul className="nice-list">{accreditations.map(x=><li key={x}>{x}</li>)}</ul></div></div></section> }
...
function ProgramCard({ program, onOpen }){
  const Icon = iconMap[program.icon] || HandHeart
  const subCategories = (program.subCategories || []).slice(0, 3)
  return <div className="program-card"><div className="program-image-wrap"><img src={program.defaultImage} alt={program.title} onError={e=>e.currentTarget.style.display='none'} /><div className="fallback"><Icon size={48}/></div></div><div className="program-body"><div className="logo-row">{program.logos.slice(0,3).map(l=><img key={l} src={l} alt="logo" onError={e=>e.currentTarget.style.display='none'} />)}</div><h3>{program.title}</h3><p>{program.overview}</p><p className="subcat-title">Sub-categories of work</p><ul className="subcat-list">{subCategories.map(item => <li key={item.title}>{item.title}</li>)}</ul><div className="sdg-row">{program.sdgs.slice(0,4).map(s=><img key={s} src={getSdgImagePath(s)} alt={`SDG ${s}`} />)}</div><button className="text-link" onClick={()=>onOpen(program.slug)}>Know More <ArrowRight size={15}/></button></div></div>
}
...
function Gallery({ program }){ const imgs=[program.defaultImage,...[1,2,3,4,5,6].map(i=>`${program.folder}${program.slug}-${i}.jpg`)]; return <div className="gallery-grid">{imgs.map((src,i)=><div className="gallery-tile" key={src+i}><img src={src} alt={`${program.title} field image ${i+1}`} onError={e=>e.currentTarget.style.display='none'} /><div className="gallery-placeholder"><ImageIcon/><small>Image not available</small></div></div>)}</div> }

function SubCategorySection({ program }){ return <section className="content-section"><div className="section-heading"><p className="eyebrow">SUB-CATEGORIES</p><h2>{program.shortTitle} programs and projects</h2></div><div className="subcat-grid">{(program.subCategories || []).map((item) => <article key={item.title} className="subcat-tile"><div className="subcat-image-wrap"><img src={item.image} alt={item.title} onError={e=>e.currentTarget.style.display='none'} /><div className="subcat-image-fallback"><ImageIcon size={20}/><span>{item.title}</span></div></div><div className="subcat-tile-body"><h3>{item.title}</h3><p>{item.description}</p><button className="text-link">Know More <ArrowRight size={14}/></button></div></article>)}</div></section> }

function ProgramDetail({ program, onBack }){ const Icon = iconMap[program.icon] || HandHeart; return <div><SiteHeader onOpenProgram={()=>{}} onGoHome={onBack}/><section className="detail-hero"><div className="detail-grid"><div><button className="back-link" onClick={onBack}>← Back to Our Work</button><div className="detail-logos"><img src={NGO_LOGO}/>{program.logos.filter(l=>l!==NGO_LOGO).map(l=><img key={l} src={l} onError={e=>e.currentTarget.style.display='none'} />)}</div><p className="eyebrow orange">OUR WORK / CORE AREA</p><h1>{program.title}</h1><p>{program.overview}</p></div><div className="hero-card"><Icon/><h3>Program Snapshot</h3><p>{program.shortTitle} at a glance with one featured image and key workstreams.</p><img className="detail-featured-image" src={program.defaultImage} alt={`${program.title} featured`} onError={e=>e.currentTarget.style.display='none'} /></div></div></section><SubCategorySection program={program}/><section className="content-section"><Gallery program={program}/></section><section className="gray-section"><div className="three-col"><InfoCard icon={Target} title="Target Beneficiaries" items={program.beneficiaries}/><InfoCard icon={BarChart3} title="Impact Metrics" items={program.metrics}/><InfoCard icon={ClipboardCheck} title="KPIs" items={program.kpis}/></div></section><section className="content-section two-card-grid"><InfoCard icon={FileText} title="Key Activities" items={program.activities}/><InfoCard icon={ShieldCheck} title="Monitoring & Evaluation" items={program.monitoring}/></section><section className="sdg-section"><p className="eyebrow">SDG ALIGNMENT</p><h2>Aligned Sustainable Development Goals</h2><div className="big-sdgs">{program.sdgs.map(s=><img key={s} src={getSdgImagePath(s)} alt={`SDG ${s}`}/>)}</div></section><Footer/></div> }
...
function Contact(){ return <section id="contact" className="content-section"><div className="cta"><h2>Let’s build stronger rural communities together.</h2><p>For CSR partnerships, donations, volunteering, project collaboration, and field implementation support, reach out to Give For Society.</p><Button>Contact Us</Button></div></section> }
