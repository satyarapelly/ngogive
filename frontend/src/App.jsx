...
function ProgramCard({ program, onOpen }){
  ...
  return <div className="program-card"><div className="program-image-wrap"><img src={program.defaultImage} alt={program.title} /></div><div className="program-body"><div className="logo-row">{program.logos.slice(0,3).map(l=><img key={l} src={l} alt="logo" />)}</div><h3>{program.title}</h3><p>{program.overview}</p><p className="subcat-title">Sub-categories of work</p><ul className="subcat-list">{subCategories.map(item => <li key={item.title}>{item.title}</li>)}</ul><div className="sdg-row">{program.sdgs.slice(0,4).map(s=><img key={s} src={getSdgImagePath(s)} alt={`SDG ${s}`} />)}</div><button className="text-link" onClick={()=>onOpen(program.slug)}>Know More <ArrowRight size={15}/></button></div></div>
}
