...
function SubCategorySection({ program }){ return <section className="content-section"><div className="section-heading"><p className="eyebrow">SUB-CATEGORIES</p><h2>{program.shortTitle} programs and projects</h2></div><div className="subcat-grid">{(program.subCategories || []).map((item) => <article key={item.title} className="subcat-tile"><div className="subcat-image-wrap"><img src={item.image} alt={item.title} onError={e=>e.currentTarget.style.display='none'} /><div className="subcat-image-fallback"><ImageIcon size={20}/><span>Image coming soon</span></div></div><div className="subcat-tile-body"><h3>{item.title}</h3><p>{item.description}</p>{item.kpiTag && <span className="subcat-kpi-tag">{item.kpiTag}</span>}{item.knowMoreLink && <a className="text-link" href={item.knowMoreLink}>Know More <ArrowRight size={14}/></a>}</div></article>)}</div></section> }

function ProgramDetail({ program, onBack }){ const Icon = iconMap[program.icon] || HandHeart; return <div><SiteHeader onOpenProgram={()=>{}} onGoHome={onBack}/><section className="detail-hero"><div className="detail-grid"><div><button className="back-link" onClick={onBack}>← Back to Our Work</button><div className="detail-logos"><img src={NGO_LOGO}/>{program.logos.filter(l=>l!==NGO_LOGO).map(l=><img key={l} src={l} onError={e=>e.currentTarget.style.display='none'} />)}</div><p className="eyebrow orange">OUR WORK / CORE AREA</p><h1>{program.title}</h1><p>{program.overview}</p></div><div className="hero-card"><Icon/><h3>Program Snapshot</h3><p>{program.shortTitle} at a glance with one featured image and key workstreams.</p><img className="detail-featured-image" src={program.defaultImage} alt={`${program.title} featured`} onError={e=>e.currentTarget.style.display='none'} /></div></div></section><SubCategorySection program={program}/><section className="gray-section"><div className="three-col"><InfoCard icon={Target} title="Target Beneficiaries" items={program.beneficiaries}/><InfoCard icon={BarChart3} title="Impact Metrics" items={program.metrics}/><InfoCard icon={ClipboardCheck} title="KPIs" items={program.kpis}/></div></section><section className="content-section two-card-grid"><InfoCard icon={FileText} title="Key Activities" items={program.activities}/><InfoCard icon={ShieldCheck} title="Monitoring & Evaluation" items={program.monitoring}/></section><section className="sdg-section"><p className="eyebrow">SDG ALIGNMENT</p><h2>Aligned Sustainable Development Goals</h2><div className="big-sdgs">{program.sdgs.map(s=><img key={s} src={getSdgImagePath(s)} alt={`SDG ${s}`}/>)}</div></section><Footer/></div> }
