...
function Gallery({ program }){ const imgs=[program.defaultImage,...[1,2,3,4,5,6].map(i=>`${program.folder}${program.slug}-${i}.jpg`)]; return <div className="gallery-grid">{imgs.map((src,i)=><div className="gallery-tile" key={src+i}><img src={src} alt="field" onError={e=>e.currentTarget.style.display='none'} /><div className="gallery-placeholder"><ImageIcon/><small>{i===0 ? program.defaultImage : program.folder}</small></div></div>)}</div> }

function ProgramDetail({ program, onBack }){ const Icon = iconMap[program.icon] || HandHeart; return <div><SiteHeader onOpenProgram={()=>{}} onGoHome={onBack}/><section className="detail-hero"><div className="detail-grid"><div><button className="back-link" onClick={onBack}>← Back to Our Work</button><div className="detail-logos"><img src={NGO_LOGO}/>{program.logos.filter(l=>l!==NGO_LOGO).map(l=><img key={l} src={l} onError={e=>e.currentTarget.style.display='none'} />)}</div><p className="eyebrow orange">OUR WORK / CORE AREA</p><h1>{program.title}</h1><p>{program.overview}</p></div><div className="hero-card"><Icon/><h3>Image Folder</h3><p className="path-text">{program.folder}</p><h4>Default Image</h4><p className="path-text">{program.defaultImage}</p></div></div></section><section className="content-section"><Gallery program={program}/></section><section className="gray-section"><div className="three-col"><InfoCard icon={Target} title="Target Beneficiaries" items={program.beneficiaries}/><InfoCard icon={BarChart3} title="Impact Metrics" items={program.metrics}/><InfoCard icon={ClipboardCheck} title="KPIs" items={program.kpis}/></div></section><section className="content-section two-card-grid"><InfoCard icon={FileText} title="Key Activities" items={program.activities}/><InfoCard icon={ShieldCheck} title="Monitoring & Evaluation" items={program.monitoring}/></section><section className="sdg-section"><p className="eyebrow">SDG ALIGNMENT</p><h2>Aligned Sustainable Development Goals</h2><div className="big-sdgs">{program.sdgs.map(s=><img key={s} src={getSdgImagePath(s)} alt={`SDG ${s}`}/>)}</div></section><Footer/></div> }
