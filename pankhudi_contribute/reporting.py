...
def write_reports(rows: list[dict[str, Any]], outdir: Path | str = "output") -> None:
    ...
    with (out/"contribution_results.csv").open("w", newline="", encoding="utf-8") as fh:
        writer = csv.DictWriter(fh, fieldnames=FIELDS); writer.writeheader(); writer.writerows(normalized)
    try:
        from openpyxl import Workbook
    except ModuleNotFoundError:
        (out/"contribution_results.xlsx.skipped.txt").write_text("openpyxl is not installed; CSV report was written instead. Run: python -m pip install -e .\n", encoding="utf-8")
    else:
        wb = Workbook(); ws = wb.active; ws.title = "Results"; ws.append(FIELDS)
        for r in normalized: ws.append([r.get(k, "") for k in FIELDS])
